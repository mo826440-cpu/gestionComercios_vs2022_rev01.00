...
public class ComercioService : IComercioService
{
    ...
    public async Task<Comercio> UpdateAsync(Comercio comercio)
    {
        var response = await _supabaseService.Client
            ...
            .Set(x => x.Email, comercio.Email ?? (string?)null)
            .Set(x => x.Telefono, comercio.Telefono ?? (string?)null)
            .Set(x => x.Direccion, comercio.Direccion ?? (string?)null)
            .Set(x => x.IdPublico, comercio.IdPublico ?? (string?)null)
